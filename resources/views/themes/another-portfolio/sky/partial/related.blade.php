<div class="flex sm:space-x-2 rtl:space-x-reverse px-2 lg:p-4 w-full">
    <a class="mb-4 md:mb-0 w-full relative h-[16em] sm:h-[20em] md:h-[22em] lg:h-[24em]" href="{{ route('post',$post->slug) }}">
        <div class="absolute inset-0 w-full h-full z-10 shadow-md rounded-lg bg-gradient-to-b from-transparent to-gray-700"></div>

        @if($post->image() !== null)
            <img alt="{{ $post->title }}" src="{{ $post->image() }}" class="absolute ltr:left-0 rtl:right-0 top-0 w-full h-full shadow-md rounded-lg z-0 object-cover"/>
        @endif
