                    ...
                    @if(isset($layout->widgets[$column->key]))
                        ...
                        @foreach($widgetsItems as $key => $data)
                            @if(class_exists($data['data']['widget']))
                                ...
                                @endphp
                                <div class="bg-white dark:bg-black shadow mb-2 md:mb-6 mt-6 mx-1 py-3 px-4 hover:shadow-lg transition-all ease-in-out duration-500 ltr:rounded-tr-none rtl:rounded-tl-none rounded-3xl border border-custom-100 dark:border-custom-700/50">
                                    @if($data['data']['title'])
